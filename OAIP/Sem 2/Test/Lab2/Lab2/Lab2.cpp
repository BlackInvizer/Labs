...
void Push(Stack** top, int border)
{
    Stack* b = new Stack;
    b->border = border;
    b->next = (*top);
...
void DirectSortByTime(InfoDesk* ID, int total) { //Сортировка методом прямого выбора
    ...
    for (int i = 0; i < total - 1; i++) {
        ...
        for (int j = i + 1; j < total; j++) {
            if (ID[j].Hour < ID[min_i].Hour || (ID[j].Hour == ID[min_i].Hour && ID[j].Minute < ID[min_i].Minute)) {
                min_i = j;
            }
        }
        InfoDesk tmp = ID[i];
...
int main()
{
    const char* FileName = "D:\\University\\OAiP\\Sem 2\\Test\\Lab2\\Infodesk.dat";
